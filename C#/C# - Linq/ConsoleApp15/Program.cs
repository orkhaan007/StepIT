...
class Program {
    ...
    static void Main(string[] args)
    {
        ...
        // var debs = debtors.Where((db) => db.Email.EndsWith("@rhyta.com") || db.Email.EndsWith("@dayrep.com"));
        // foreach (var db in debs) 
        //     Console.WriteLine(db);
        //
        // var debs2 = debtors.Where((db) => DateAndTime.Now.Year - db.BirthDay.Year <= 26 || DateAndTime.Now.Year - db.BirthDay.Year >= 36);
        // foreach (var db in debs2) 
        //     Console.WriteLine(db);
        //
        // var debs3 = debtors.Where((db) => db.Debt <= 5000);
        // foreach (var db in debs3) 
        //     Console.WriteLine(db);
        //
        // var debs4 = debtors.Where((db) => db.FullName.Length > 18 && db.Phone.Count(phone=> phone== '7') > 2);
        // foreach (var db in debs4) 
        //     Console.WriteLine(db);
        
        // var debs5 = debtors.Where((db) => db.BirthDay.Month == 12 || db.BirthDay.Month == 1 || db.BirthDay.Month == 2);
        // foreach (var db in debs5) 
        //     Console.WriteLine(db);
        //
        // double averageDebt = debtors.Average(db => db.Debt);
        // var debs6 = debtors.Where(db => db.Debt > averageDebt)
        //     .OrderBy(debtor => debtor.FullName)
        //     .ThenByDescending(debtor => debtor.Debt);
        // foreach (var db in debs6)
        //     Console.WriteLine(db);
        
        // var debtorsWithoutEightInPhoneNumber = debtors.Where(debtor => !debtor.Phone.Contains('8')).Select(debtor => new { debtor.FullName, Age = DateTime.Now.Year - debtor.BirthDay.Year, debtor.Debt }).ToList();
        // foreach (var debtor in debtorsWithoutEightInPhoneNumber)
        // {
        //     Console.WriteLine($"{debtor.FullName}, {debtor.Age} years old, Debt: {debtor.Debt}");
        // }
        //
        // var debtorsWithThreeSameLettersInName = debtors.Where(debtor => debtor.FullName.GroupBy(c => c).Any(group => group.Count() >= 3)).OrderBy(debtor => debtor.FullName).ToList();
        // foreach (var debtor in debtorsWithThreeSameLettersInName)
        // {
        //     Console.WriteLine(debtor.ToString());
        // }
        //
        // var mostCommonBirthYear = debtors.GroupBy(debtor => debtor.BirthDay.Year).OrderByDescending(group => group.Count()).First().Key;
        // Console.WriteLine($"Most common birth year: {mostCommonBirthYear}");
        //
        // var top5Debtors = debtors.OrderByDescending(debtor => debtor.Debt).Take(5).ToList();
        // foreach (var debtor in top5Debtors)
        // {
        //     Console.WriteLine(debtor.ToString());
        // }
        //
        // var totalDebt = debtors.Sum(debtor => debtor.Debt);
        // Console.WriteLine($"Total debt of all debtors: {totalDebt}");
        //
        // var ww2Veterans = debtors.Where(debtor => debtor.BirthDay.Year < 1946).ToList();
        // foreach (var debtor in ww2Veterans)
        // {
        //     Console.WriteLine(debtor.ToString());
        // }
        //
        // var debtorsWithoutRepeatedNumbers = debtors.Where(debtor => debtor.Phone.Distinct().Count() == debtor.Phone.Length).Select(debtor => new { debtor.FullName, debtor.Debt }).ToList();
        // foreach (var debtor in debtorsWithoutRepeatedNumbers)
        // {
        //     Console.WriteLine($"{debtor.FullName}, Debt: {debtor.Debt}");
        // }
        //
        // var debtorsPayingOffDebt = debtors.Where(debtor => debtor.BirthDay.DayOfYear >= DateTime.Now.DayOfYear && debtor.Debt <= (500 * (debtor.BirthDay.Year - DateTime.Now.Year))).ToList();
        // foreach (var debtor in debtorsPayingOffDebt)
        // {
        //     Console.WriteLine(debtor.ToString());
        // }
        //
        // var debtorsWithSmileName = debtors.Where(debtor => debtor.FullName.Contains("s") && debtor.FullName.Contains("m") && debtor.FullName.Contains("i") && debtor.FullName.Contains("l") && debtor.FullName.Contains("e")).ToList();
        // foreach (var debtor in debtorsWithSmileName)
        // {
        //     Console.WriteLine(debtor.ToString());
        // }
